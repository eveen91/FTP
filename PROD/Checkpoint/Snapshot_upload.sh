...
# ==========================================
# CZĘŚĆ 0: SPRAWDZENIE FOLDERU UPLOAD (KOREKTA LOGIKI)
# ==========================================
...
if [ -n "$(ls -A "$UPLOAD_FOLDER" 2> /dev/null)" ]; then
  ...
  log_msg "Znaleziono ${#files_to_upload[@]} pliki w folderze upload/ - dodane do wysyłki."
else
  log_msg "Folder '$UPLOAD_FOLDER' jest pusty. Wczytujemy nowe snapshoty..."
fi

# ==========================================
# CZĘŚĆ 1: ANALIZA I PRZENOSZENIE STARYCH PLIKÓW (TYLKO DLA NOWYCH)
# ==========================================
declare -A best_files # Najnowsze snapshoty per system
# Uwaga: Pobieramy pliki tylko z katalogu bieżącego, nie z upload/
...
if [ ${#files_to_upload[@]} -eq 0 ]; then
  ...
  # Przechodzimy przez każdą pozycję w tablicy files_to_upload
