...
log "Starting backup sort in '$backup_dir', older files go to '$upload_dir'"
...
while IFS= read -r -d '' file; do
  ...
  log "Checking '$base' (system: $firewall_name, modified: $(date -d "@$mtime" '+%Y-%m-%d %H:%M:%S'))"
  ...
  if [[ -z "${newest_time[$firewall_name]+x}" ]] || ((mtime > newest_time[$firewall_name])); then
    ...
    if [[ -n "${newest_file[$firewall_name]+x}" ]]; then
      ...
      log "'$base' is newer than previous newest '$old_base' for '$firewall_name'"
      echo "    -> moving superseded file '$old_base' to '$upload_dir/'"
      log "moving superseded file '$old_base' to '$upload_dir'"
      mv "${newest_file[$firewall_name]}" "$upload_dir/"
      moved_count=$((moved_count + 1))
    else
      echo "    -> first file seen for system '$firewall_name', keeping in place for now"
      log "first file seen for system '$firewall_name', keeping in place for now"
    fi
    ...
    log "'$base' is older than current newest for '$firewall_name', moving to '$upload_dir'"
    mv "$file" "$upload_dir/"
    moved_count=$((moved_count + 1))
  fi
done < "$file_list"
...
log "Done. Processed $file_count file(s), moved $moved_count to '$upload_dir'."

if ((file_count > 0)); then
  echo "==> Uploading files in '$upload_dir' to the SFTP server."
  log "Uploading files in '$upload_dir' to the SFTP server."
  ...
  # Function to check if files exist on the SFTP server and delete them from local host
  function check_and_delete_files {
    for file in "$upload_dir"/*; do
      if [[ -f "$file" ]]; then
        base=$(basename "$file")
        ...
        if ssh -i "$ssh_key" "$sftp_user@$sftp_server" test -e "$remote_file"; then
          echo "File '$base' exists on the SFTP server. Deleting from local host."
          log "File '$base' exists on the SFTP server. Deleting from local host."
          rm "$file"
        else
          echo "File '$base' does not exist on the SFTP server."
          log "File '$base' does not exist on the SFTP server."
        fi
      fi
    done
  }
  ...
  echo "==> Uploaded all files to the SFTP server and verified existence, then deleted from local host."
  log "Uploaded all files to the SFTP server and verified existence, then deleted from local host."
else
  echo "==> No matching backup files found or no files were moved for upload."
  log "No matching backup files found or no files were moved for upload."
fi
